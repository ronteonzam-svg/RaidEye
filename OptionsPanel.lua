			...
			colors = {
				...
				args = {
					...
					opacity = {
						...
						order = 15,
						get = function()
							return tonumber((("%%." .. "%d" .. "f"):format(2)):format(1 - self.db.profile[i].opacity))
						end,
						set = function(_, val)
							self.db.profile[i].opacity = tonumber((("%%." .. "%d" .. "f"):format(2)):format(1 - val))
							for j = 1, #self.groups do
								if j == i or self.db.profile[j].inherit == i then
									for k = 1, #self.groups[j].CooldownFrames do
...
for spellID, spellConfig in pairs(self.spells) do
	if not spellConfig.parent then
		...
		if classGroup then
			...
			if icon then
				displayName = ("|T%s:16|t %s"):format(icon, name)
			end
